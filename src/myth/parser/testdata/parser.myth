...
// This is a modified version of the Go language parser
// See github.com/golang/go/LICENSE for more details.

// Package parser implements a parser for Myth source files. Input may be
// provided in a variety of forms (see the various Parse* functions); the
// output is an abstract syntax tree (AST) representing the Myth source. The
// parser is invoked through one of the Parse* functions.
//
// The parser accepts a larger language than is syntactically permitted by
// the Myth spec, for simplicity, and for improved robustness in the presence
...
func syncStmt(p *parser) {
	for {
		...
		case token.BREAK, token.CONST, token.CONTINUE, token.DEFER,
			token.FALLTHROUGH, token.FOR, token.GOTO,
...
func (p *parser) parseStmt() (s ast.Stmt) {
	...
	case
		...
		token.IDENT, token.INT, token.FLOAT, token.CHAR, token.STRING, token.FUNC, token.LPAREN, // operands
